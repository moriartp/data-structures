...
$('tbody').find('tr').each(function(i, elem){
        ...
        meetings.push($(elem).find('td').eq(1).html().split('</b> ')[1].trim());        
        meetings.push($(elem).find('td').eq(1).html().split('</b> ')[2].trim());
    });

console.log(meetings.length); // print number of meetings in meetings array
fs.writeFileSync('/home/ubuntu/workspace/week5/data/meetingsArrayTime.txt', JSON.stringify(meetings));